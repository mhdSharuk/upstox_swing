...
class SupabaseStorage:
    ...
    def upload_signal_parquet(self, df: pd.DataFrame, timeframe: str) -> bool:
        try:
            signal_key = f'{timeframe}_signals'
            filename = SUPABASE_CONFIG['file_names'].get(signal_key)
            if not filename:
                logger.error(f"No filename configured for signal: {signal_key}")
                return False
            ...
            file_size_kb = (len(buffer.getvalue()) / 1024) / 1024
            logger.info(f"  Signal parquet file size: {file_size_kb:.2f} MB")
            ...
            logger.error(f"Error uploading signal parquet file: {e}")
            return False

    def upload_all_signal_timeframes(self, signals_dict: Dict[str, pd.DataFrame]) -> bool:
        if not self.client:
            logger.error("Not authenticated. Call authenticate() first.")
            return False
        
        logger.info("\n" + "=" * 60)
        logger.info("UPLOADING SIGNAL PARQUET FILES TO SUPABASE STORAGE")
        logger.info("=" * 60)
        logger.info(f"Bucket: {self.bucket_name} (PUBLIC)")
        logger.info(f"Signal timeframes to upload: {list(signals_dict.keys())}")
        logger.info("=" * 60 + "\n")
        
        all_success = True
        
        for timeframe, df in signals_dict.items():
            success = self.upload_signal_parquet(df, timeframe)
            
            if not success:
                all_success = False
                logger.error(f"Failed to upload {timeframe} signal data")
        
        if all_success:
            logger.info("\n✓ All signal parquet files uploaded successfully!")
            logger.info(f"✓ Files are publicly accessible at:")
            logger.info(f"   {self.url}/storage/v1/object/public/{self.bucket_name}/")
        else:
            logger.error("\n✗ Some signal parquet files failed to upload")
